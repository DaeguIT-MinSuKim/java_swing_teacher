package java_swing_teacher.chap14;
...
import java_swing_teacher.chap14.exam.ui.panel.StudentPanel;
...
public class OptionPaneEx extends JFrame implements ActionListener {
	...
	public OptionPaneEx() {
		chooser = new JFileChooser(System.getProperty("usr.dir"));

		initialize();
	}

	private void initialize() {
		setTitle("팝업다이얼로그, 파일다얼로그, 컬러다얼로그");
		...
		pFileDlg = new JPanel();
		pFileDlg.setBorder(
				new TitledBorder(null, "\uD30C\uC77C\uC5F4\uAE30/\uC800\uC7A5 \uB2E4\uC774\uC5BC\uB85C\uADF8",
		...
		lblColor.setFont(new Font("휴먼둥근헤드라인", Font.BOLD | Font.ITALIC, 20));
	...
	protected void btnPopupInputActionPerformed(ActionEvent e) {
		/*
		 * parentComponent the parent Component for thedialog message the Object to
		 * display title the String to display in thedialog title bar messageType the
		 * type of message to be displayed: ERROR_MESSAGE, INFORMATION_MESSAGE,
		 * WARNING_MESSAGE, QUESTION_MESSAGE,or PLAIN_MESSAGEicon the Icon image to
		 * display selectionValues an array of Objects that gives the possible
		 * selectionsinitial SelectionValue the value used to initialize the inputfield
		 */
